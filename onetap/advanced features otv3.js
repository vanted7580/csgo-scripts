...
function create_string(x, y, aligned, text, color, isTitle) {
    if (font == 0 || font == Render.AddFont(style.title_font, style.title_font_size,20)) {
        //font = Render.AddFont(style.main_font, style.main_font_size,20);
    }
    if (isTitle) {
        font = Render.AddFont(style.title_font, style.title_font_size,20);
    }
...
function begin_menu(title, x, y, width, height) {
    ...
    {
        const texture = Render.AddTexture("ot/pictures/bg_eva.jpg");
...
function create_key_binds( title, x, y ,options,disable_print ) //options= [key,state,type,last state], return options
{
    ...
    if ((inbox(x + 1, y - 4, x + 24 + get_textsize(title).x + 35, y + 19) || options[0] == "wait" )&&disable_print) {
        if( !disable_print )
        {
...
var rage_subtab3_var =
{
    ...
    sub2_2: [0,["Off","+2 ticks","+4 ticks","+6 ticks","+8 ticks","+10 ticks"],false],  
    sub3: 0,
    sub3_1: 0,
    //sub3_2: 0,
...
function menu()
{
    ...
    //menu open
    if ( !UI.IsMenuOpen() && initialize_alpha > 0 )
    {
        return;
    }
    ...
    if ( PREVIEW_OPEN )
    {
        Render.String(windows.x+ 200, windows.y + menu_size.y - 20, 0, "Preview version, not represent the final quality", [255,255,255,initialize_alpha], Render.AddFont(style.main_font, 12, 20)); 
    }
    ...
    { 
        ...
        if ( inbox(help_x-6,help_y-6,help_x+6,help_y+6) && initialize_alpha > 254 && initialize_adm && !reload_now)
        {
            ...
            Render.String(help_x-1.5, help_y-7, 0, "", [0,0,0,initialize_alpha], Render.AddFont(style.main_font, 10, 20)); 

            /*

            //Render.Line(help_x, help_y, x -2, y + 15, style.window_border_col);
            Render.FilledRect(help_x-15, help_y+24, menu_size.x, 56, style.window_bg_col);

            Render.FilledRect(help_x - 14, help_y + 22, menu_size.x - 2, 2, style.window_bg_col);
            Render.FilledRect(help_x - 13, help_y + 21, menu_size.x - 4, 1, style.window_bg_col);
            Render.FilledRect(help_x - 12, help_y + 20, menu_size.x - 8, 1, style.window_bg_col);

            
            Render.FilledRect(help_x - 14, help_y + 80, menu_size.x - 2, 2, style.window_bg_col);
            Render.FilledRect(help_x -13, help_y + 80, menu_size.x - 4, 3, style.window_bg_col);
            Render.FilledRect(help_x - 12, help_y + 80, menu_size.x - 8, 4, style.window_bg_col);

            Render.String(help_x - 10, help_y + 32, 0, " | If this js is not working well, you shoud: ", style.font_hover_col, Render.AddFont(style.main_font, 14, 500)); 
            Render.String(help_x - 10, help_y + 53, 0, " - Onetap -> Username -> Scripts -> Reload all", style.font_hover_col, Render.AddFont(style.main_font, 14, 500)); 
            */
            ...
            else
            {
                ...
                else
                {
                    ...
                    Render.String(help_x + 28, help_y-15, 0, "Hold to reload.", style.font_hover_col, Render.AddFont(style.main_font, 10, 20)); 
        ...
        else
        {
            ...
            Render.String(help_x-1.5, help_y-7, 0, "?", [128,128,128,initialize_alpha], Render.AddFont(style.main_font, 10, 20)); 
        }
    }
        ...
        if ( menu_opened.tab_user )
        {
            ...
            if ( subtab_opend.user_subtab1 )
            {
                ...
                if ( initialize_alpha > 128 && TEXTURE_LOGO)
                {
                    ...
                    var font = Render.AddFont(style.title_font, 20, 900)
...
function rage_subtab3_doubletap() // some code from Alpha_DT cracked
{
    ...
    if ( rage_subtab3_var.sub1[0] > 0 )
    {
        ...
        if ( !doubletap_key && !recharge_set_once )
        {
            ...
            value_d = setDropdownValue(value_d, 2,true);
            value_d = setDropdownValue(value_d, 3, true);
            value_d = setDropdownValue(value_d, 4, true);
        ...
        if ( doubletap_key && !hideshots_key )
        {
            ...
            if ( rage_subtab3_var.sub1[0] == 1 )
            {
                ...
                if ( rage_subtab3_var.sub2[0] == 0 )
                {
                    ...
                    dis = distance - 300;
                    tolerance = Math.floor( dis / 100 ) - 1;
                    ...
                    var max = 14 + ( rage_subtab3_var.sub2_1[0] * 2 );
...
{
    if ( false && rage_subtab3_var.sub1[0] == 1 && Entity.IsAlive(Entity.GetLocalPlayer()) )
    {
        var font = Render.AddFont("Arialbd.ttf", 15, 900)
        distance = Math.floor(distance);
        Render.String((ScreenSize[0] / 2) - 26, (ScreenSize[1] / 2) - 30, 0, shift.toString() + " " + tolerance.toString(), [255,255,255,255], font)
        Render.String((ScreenSize[0] / 2) + 5, (ScreenSize[1] / 2) - 30 , 0, distance.toString() , [255,255,255,255], font)
...
function rage_subtab4_fix_hs()
{
    ...
    if ( rage_subtab4_var.sub2 && false)
    {
    ...
    switch( misc_subtab2_var.sub1[0] )
    {
        ...
        case 2:
            ...
            */
            ...
            var start_x = ScreenSize[0] - 100+30;
            var start_y = 6;
            var end_x = ScreenSize[0];
            var end_y = start_y+100+8;
            
            Render.GradientRect(start_x, start_y, end_x-start_x, end_y-start_y, 1, [ 0, 0, 0,0], [0 , 0, 0, 152]);

            Render.String(start_x+1, start_y+3, 0, text, [0, 0, 0, 146], Render.AddFont(style.title_font, 12, 40));
            Render.String(start_x, start_y+2, 0, text, [255, 255, 255, 255], Render.AddFont(style.title_font, 12, 40));
