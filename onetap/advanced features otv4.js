...
function create_slider(title, x, y, max, value) {
    ...
    if ((inbox(x, y + 10, x + 350, y + 23) || sliding) && Input.IsKeyPressed(0x01)) {
        ...
        if (Input.IsKeyPressed(0x01)&&inbox(x, y + 10, x + 350, y + 23) ) {
            sliding = true;
...
function create_key_binds( title, x, y ,options ) //options= [key,state,type,last state], return options
{
    ...
    if (inbox(x + 1, y - 4, x + 24 + get_textsize(title).x + 35, y + 19) || options[0] == "wait" ) {
...
{
    UI.AddSliderInt(["Config", "AdvancedFeatures","AdvancedFeatures",], name+"_key", -1, 51);
    UI.AddSliderInt(["Config", "AdvancedFeatures","AdvancedFeatures",], name+"_type", -1, 3);
}

function save_key( name, options )
{
    UI.SetValue(["Config", "AdvancedFeatures","AdvancedFeatures", name+"_key"], get_key_id(options[0]));
    UI.SetValue(["Config", "AdvancedFeatures","AdvancedFeatures", name+"_type"], options[2]);
    UI.SetEnabled(["Config", "AdvancedFeatures","AdvancedFeatures", name+"_key"],0);
    UI.SetEnabled(["Config", "AdvancedFeatures","AdvancedFeatures", name+"_type"],0);
}

function get_key( name, options )
{
    options[0] = key_map[UI.GetValue(["Config", "AdvancedFeatures","AdvancedFeatures", name+"_key"])];
    options[2] = UI.GetValue(["Config", "AdvancedFeatures","AdvancedFeatures", name+"_type"]);
    return options;
}
...
UI.AddSubTab(["Config", "SUBTAB_MGR"], "AdvancedFeatures");

const user_database = ["wqy224491","MadMan121","aD2077"];
const user_name = Cheat.GetUsername();
var initialized = 0;
...
function initialize()
{
    ...
    if( !initialize_adm  )
    {
        ...
        {
            initialize_alpha_load = normalize_alpha((Global.Realtime()-(initialize_load_time-2.52))*500,255);
        }
        else
        {
            initialize_alpha = 0;
...
{
    sub1: [0,["Off","HvH Server","MM Server","Manual"],false],
    sub2: [0,["Prefer speed", "Prefer accuracy"],false],
    sub2_1: [0,["Off","+2 ticks","+4 ticks","+6 ticks"],false],
    sub3: 0,
    sub3_1  : 0
}
...
var last_connect_state = is_connected();
var menu_subtabs_last_state = false
...
UI.AddDropdown (["Config", "AdvancedFeatures","AdvancedFeatures"], user_subtab3_name.sub1, ["1","2","3","4"], 0);
UI.AddCheckbox (["Config", "AdvancedFeatures","AdvancedFeatures"], user_subtab3_name.sub2);
UI.AddDropdown (["Config", "AdvancedFeatures","AdvancedFeatures"], user_subtab3_name.sub3, ["1","2","3","4"], 0);
UI.AddDropdown (["Config", "AdvancedFeatures","AdvancedFeatures"], user_subtab3_name.sub4, ["1","2","3","4"], 0);
UI.SetEnabled( ["Config", "AdvancedFeatures","AdvancedFeatures", user_subtab3_name.sub1], 0 );
UI.SetEnabled( ["Config", "AdvancedFeatures","AdvancedFeatures", user_subtab3_name.sub2], 0 );
UI.SetEnabled( ["Config", "AdvancedFeatures","AdvancedFeatures", user_subtab3_name.sub3], 0 );
UI.SetEnabled( ["Config", "AdvancedFeatures","AdvancedFeatures", user_subtab3_name.sub4], 0 );
...
function menu()
{
    ...
    //menu open
    if ( !UI.IsMenuOpen() && initialize_adm )
    {
        return;
    }
    ...
    if ( initialized == 1 )
    {
        ...
        if( !menu_subtabs_last_state )
        {
            menu_subtabs_last_state = true;
            user_subtab3_var.sub1[0] = UI.GetValue(["Config", "AdvancedFeatures","AdvancedFeatures", user_subtab3_name.sub1]);
            user_subtab3_var.sub2 = UI.GetValue(["Config", "AdvancedFeatures","AdvancedFeatures", user_subtab3_name.sub2]);
            user_subtab3_var.sub3[0] = UI.GetValue(["Config", "AdvancedFeatures","AdvancedFeatures", user_subtab3_name.sub3]);
            user_subtab3_var.sub4[0] = UI.GetValue(["Config", "AdvancedFeatures","AdvancedFeatures", user_subtab3_name.sub4]);
        }
        else
        {
            UI.SetValue(["Config", "AdvancedFeatures","AdvancedFeatures", user_subtab3_name.sub1] , user_subtab3_var.sub1[0] );
            UI.SetValue(["Config", "AdvancedFeatures","AdvancedFeatures", user_subtab3_name.sub2] , bool_to_int(user_subtab3_var.sub2) );
            UI.SetValue(["Config", "AdvancedFeatures","AdvancedFeatures", user_subtab3_name.sub3] , user_subtab3_var.sub3[0] );
            UI.SetValue(["Config", "AdvancedFeatures","AdvancedFeatures", user_subtab3_name.sub4] , user_subtab3_var.sub4[0] );
        } 

        if ( is_connected() !=last_connect_state )
        {
            globaltime = Globals.Realtime();

            last_connect_state = is_connected();
            ...
            //create_string(windows.x + 160, windows.y + 85, 0,  "Error! This page is not running! ", style.font_col);
            //create_string(windows.x + 160, windows.y + 115, 0,  " | Why this happend?", style.font_col);
            //create_string(windows.x + 160, windows.y + 135, 0, " - There was a bug on draw fn when changing server.", style.font_col);
            //create_string(windows.x + 160, windows.y + 160, 0, " - This bug caused this page crashed and not working", style.font_col);
            //create_string(windows.x + 160, windows.y + 180, 0, "    anymore.", style.font_col);
            //create_string(windows.x + 160, windows.y + 205, 0, " | How can I fix it?", style.font_col);
            //create_string(windows.x + 160, windows.y + 225, 0, " - Please relaoad this script after changing server!", style.font_col);
            //create_string(windows.x + 160, windows.y + 245, 0, " - Onetap -> Username -> Scripts -> Reload all", style.font_col);
            //create_string(windows.x + 160, windows.y + 265, 0, " - All your config will be saved!", style.font_col);
        ...
        if ( menu_opened.tab_rage )
        {
            ...
            if (create_tab(subtab_name.rage_subtab3, windows.x + 30 , windows.y + 150, subtab_opend.rage_subtab3)) {
                clear_subtabs("rage_subtab");
                subtab_opend.rage_subtab3 = true;
            }
            if (create_tab(subtab_name.rage_subtab4, windows.x + 30 , windows.y + 180, subtab_opend.rage_subtab4)) {
                clear_subtabs("rage_subtab");
                ...
                switch( rage_subtab3_var.sub1[0] )
                {
                    ...
                    case 1:
                        if ( !rage_subtab3_var.sub1[2] )
                        {
                            ...
                            if ( !rage_subtab3_var.sub2[2] && rage_subtab3_var.sub2[0] == 0 )
                            {
        ...
        if ( menu_opened.tab_aa )
        {
            ...
            {
                create_string(windows.x + 170, windows.y + 85, 0, subtab_name.aa_subtab3, style.font_col);
            }
        }
        //visuals
        ...
        if ( menu_opened.tab_user )
        {
            ...
            if ( subtab_opend.user_subtab1 )
            {
                ...
                create_string(windows.x + 165, windows.y + 140, 0, " | Created by", style.font_col);
                create_string(windows.x + 185, windows.y + 160, 0, " - VANTED ( Bilibili UID: 94341724 )", style.font_col);
                create_string(windows.x + 165, windows.y + 185, 0, " | Supported by", style.font_col);
                create_string(windows.x + 185, windows.y + 205, 0, " - Futuresense by AD#1337", style.font_col);
                create_string(windows.x + 185, windows.y + 225, 0, " - MSR GAMING GROUP", style.font_col);
                create_string(windows.x + 185, windows.y + 245, 0, " - And You", style.font_col);
                create_string(windows.x + 165, windows.y + 280, 0, " | Codes by", style.font_col);
                create_string(windows.x + 185, windows.y + 305, 0, " - Basic GUI Module by supplybarren", style.font_col);
                create_string(windows.x + 185, windows.y + 325, 0, " - Advanced GUI by VANTED", style.font_col);
            }   
            ...
            if ( subtab_opend.user_subtab4 )
            {
                ...
                create_string(windows.x + 205, windows.y + 125, 0, " - Tencent Chat Group: 745124915", style.font_col);
            } 
        }
    }
...
function user_subtab3()
{
    ...
    switch (user_subtab3_var.sub4[0])
    {
        case 0:
...
UI.AddDropdown (["Config", "AdvancedFeatures","AdvancedFeatures"], rage_subtab3_name.sub1, ["1", "2","3"], 0);
UI.AddDropdown (["Config", "AdvancedFeatures","AdvancedFeatures"], rage_subtab3_name.sub2, ["1", "1"], 0);
UI.AddDropdown (["Config", "AdvancedFeatures","AdvancedFeatures"], rage_subtab3_name.sub2_1, ["0", "1","2","3"], 0);
UI.AddSliderInt(["Config", "AdvancedFeatures","AdvancedFeatures"], rage_subtab3_name.sub3, 0, 64);
UI.AddSliderInt(["Config", "AdvancedFeatures","AdvancedFeatures"], rage_subtab3_name.sub3_1, 0, 8);

UI.SetEnabled( ["Config", "AdvancedFeatures","AdvancedFeatures", rage_subtab3_name.sub1], 0 );
UI.SetEnabled( ["Config", "AdvancedFeatures","AdvancedFeatures", rage_subtab3_name.sub2], 0 );
UI.SetEnabled( ["Config", "AdvancedFeatures","AdvancedFeatures", rage_subtab3_name.sub2_1], 0 );
UI.SetEnabled( ["Config", "AdvancedFeatures","AdvancedFeatures", rage_subtab3_name.sub3], 0 );
UI.SetEnabled( ["Config", "AdvancedFeatures","AdvancedFeatures", rage_subtab3_name.sub3_1], 0 );
...
function rage_subtab3()
{
    ...
    if( !rage_subtab3_last_state )
    {
        ...
        rage_subtab3_var.sub1[0] = UI.GetValue(["Config", "AdvancedFeatures","AdvancedFeatures", rage_subtab3_name.sub1]);
        rage_subtab3_var.sub2[0] = UI.GetValue(["Config", "AdvancedFeatures","AdvancedFeatures", rage_subtab3_name.sub2]);
        rage_subtab3_var.sub2_1[0] = UI.GetValue(["Config", "AdvancedFeatures","AdvancedFeatures", rage_subtab3_name.sub2_1]);
        rage_subtab3_var.sub3 = UI.GetValue(["Config", "AdvancedFeatures","AdvancedFeatures", rage_subtab3_name.sub3]);
        rage_subtab3_var.sub3_1 = UI.GetValue(["Config", "AdvancedFeatures","AdvancedFeatures", rage_subtab3_name.sub3_1]);
    }
    else
    {
        UI.SetValue(["Config", "AdvancedFeatures","AdvancedFeatures", rage_subtab3_name.sub1] , rage_subtab3_var.sub1[0] );
        UI.SetValue(["Config", "AdvancedFeatures","AdvancedFeatures", rage_subtab3_name.sub2] , rage_subtab3_var.sub2[0] );
        UI.SetValue(["Config", "AdvancedFeatures","AdvancedFeatures", rage_subtab3_name.sub2_1] , rage_subtab3_var.sub2_1[0] );
        UI.SetValue(["Config", "AdvancedFeatures","AdvancedFeatures", rage_subtab3_name.sub3] , rage_subtab3_var.sub3 );
        UI.SetValue(["Config", "AdvancedFeatures","AdvancedFeatures", rage_subtab3_name.sub3_1] , rage_subtab3_var.sub3_1 );
    }

    if ( rage_subtab3_var.sub1[0] != 0 )
    {
        UI.SetValue(["Rage", "Exploits", "Speed"], 3);
    }
...
UI.AddCheckbox (["Config", "AdvancedFeatures","AdvancedFeatures"], rage_subtab4_name.sub1);
UI.SetEnabled( ["Config", "AdvancedFeatures","AdvancedFeatures", rage_subtab4_name.sub1], 0 );
...
function rage_subtab4()
{
    ...
    if( !rage_subtab4_last_state )
    {
        ...
        rage_subtab4_var.sub1 = UI.GetValue(["Config", "AdvancedFeatures","AdvancedFeatures", rage_subtab4_name.sub1]);
    }
    else
    {
        UI.SetValue(["Config", "AdvancedFeatures","AdvancedFeatures", rage_subtab4_name.sub1] , bool_to_int(rage_subtab4_var.sub1));
    }
...
function rage_subtab3_doubletap()
{

    key = UI.GetValue(['Rage','Exploits','Keys','Key assignment','Double tap']);

    if ( rage_subtab4_var.sub1 > 0 )
    {
        if ( key )
        {
            ...
            if ( can_shift_shot(16) && Exploit.GetCharge() != 1 && (!Entity.IsAlive(target) && !Entity.IsDormant(target) || target == null ))
            {
            ...
            if ( rage_subtab3_var.sub1[0] == 1 )
            {
                ...
                if ( rage_subtab3_var.sub2[0] == 0 )
                {
                    ...
                    shift = 14 + ( rage_subtab3_var.sub2_1[0] * 2 ) - tolerance;
                ...
                Exploit.OverrideMaxProcessTicks(shift);
            ...
            else if( rage_subtab3_var.sub1[0] == 2 )
            {
                ...
                Exploit.OverrideMaxProcessTicks(8);
            }
            else if( rage_subtab3_var.sub1[0] == 3 )
            {
                ...
                Exploit.OverrideShift(shift);
                Exploit.OverrideTolerance(tolerance);
                Exploit.OverrideMaxProcessTicks(shift_ticks+2);
            }
        }
    }
    else
...
function draw()
{
    if ( UI.GetValue(['Rage','Exploits','Keys','Key assignment','Double tap']) && rage_subtab3_var.sub1[0] > 0 )
    {
        var font = Render.AddFont("Arialbd.ttf", 15, 900)
        distance = Math.floor(distance);
        Render.String((ScreenSize[0] / 2) - 26, (ScreenSize[1] / 2) - 20, 0, shift.toString() + " " + tolerance.toString(), [255,255,255,255], font)
        Render.String((ScreenSize[0] / 2) + 5, (ScreenSize[1] / 2) - 20 , 0, distance.toString() , [255,255,255,255], font)
...
UI.AddDropdown (["Config", "AdvancedFeatures","AdvancedFeatures"], misc_subtab2_name.sub1, ["1", "2","3"], 0);
UI.AddCheckbox (["Config", "AdvancedFeatures","AdvancedFeatures"], misc_subtab2_name.sub1_1);
UI.AddCheckbox (["Config", "AdvancedFeatures","AdvancedFeatures"], misc_subtab2_name.sub2);
UI.AddCheckbox (["Config", "AdvancedFeatures","AdvancedFeatures"], misc_subtab2_name.sub3);
UI.AddDropdown (["Config", "AdvancedFeatures","AdvancedFeatures"], misc_subtab2_name.sub4, ["1", "2","3"], 0);
UI.AddDropdown (["Config", "AdvancedFeatures","AdvancedFeatures"], misc_subtab2_name.sub5, ["1", "2","3","4","5"], 0);
const online_players = UI.AddDropdown(["Config", "AdvancedFeatures","AdvancedFeatures"],  misc_subtab2_name.sub5_1, [], 1);
UI.SetEnabled( ["Config", "AdvancedFeatures","AdvancedFeatures", misc_subtab2_name.sub1], 0 );
UI.SetEnabled( ["Config", "AdvancedFeatures","AdvancedFeatures", misc_subtab2_name.sub1_1], 0 );
UI.SetEnabled( ["Config", "AdvancedFeatures","AdvancedFeatures", misc_subtab2_name.sub2], 0 );
UI.SetEnabled( ["Config", "AdvancedFeatures","AdvancedFeatures", misc_subtab2_name.sub3], 0 );
UI.SetEnabled( ["Config", "AdvancedFeatures","AdvancedFeatures", misc_subtab2_name.sub4], 0 );
UI.SetEnabled( ["Config", "AdvancedFeatures","AdvancedFeatures", misc_subtab2_name.sub5], 0 );
UI.SetEnabled( ["Config", "AdvancedFeatures","AdvancedFeatures",  misc_subtab2_name.sub5_1], 0 );
UI.SetValue(["Config", "AdvancedFeatures","AdvancedFeatures", misc_subtab2_name.sub5_1] , misc_subtab2_var.sub5_1[0] );
...
function log_doubletap_speed()
{
    if ( misc_subtab2_name.sub1_1 )
    {
        if ( UI.GetValue(["Rage", "Exploits", "Keys", "Double tap"]) )
        {
            ...
            if ( Exploit.GetCharge() >= 0.99999999 )
            {
...
function misc_subtab2()
{
    ...
    if( !misc_subtab2_last_state )
    {
        ...
        misc_subtab2_var.sub1[0] = UI.GetValue(["Config", "AdvancedFeatures","AdvancedFeatures", misc_subtab2_name.sub1]);
        misc_subtab2_var.sub1_1 = UI.GetValue(["Config", "AdvancedFeatures","AdvancedFeatures", misc_subtab2_name.sub1_1]);
        misc_subtab2_var.sub2 = UI.GetValue(["Config", "AdvancedFeatures","AdvancedFeatures", misc_subtab2_name.sub2]);
        misc_subtab2_var.sub3 = UI.GetValue(["Config", "AdvancedFeatures","AdvancedFeatures", misc_subtab2_name.sub3]);
        misc_subtab2_var.sub4[0] = UI.GetValue(["Config", "AdvancedFeatures","AdvancedFeatures", misc_subtab2_name.sub4]);
        misc_subtab2_var.sub5[0] = UI.GetValue(["Config", "AdvancedFeatures","AdvancedFeatures", misc_subtab2_name.sub5]);
    }
    else
    {
        UI.SetValue(["Config", "AdvancedFeatures","AdvancedFeatures", misc_subtab2_name.sub1] , misc_subtab2_var.sub1[0] );
        UI.SetValue(["Config", "AdvancedFeatures","AdvancedFeatures", misc_subtab2_name.sub1_1] , bool_to_int(misc_subtab2_var.sub1_1) );
        UI.SetValue(["Config", "AdvancedFeatures","AdvancedFeatures", misc_subtab2_name.sub2] , bool_to_int(misc_subtab2_var.sub2) );
        UI.SetValue(["Config", "AdvancedFeatures","AdvancedFeatures", misc_subtab2_name.sub3] , bool_to_int(misc_subtab2_var.sub3) );
        UI.SetValue(["Config", "AdvancedFeatures","AdvancedFeatures", misc_subtab2_name.sub4] , misc_subtab2_var.sub4[0] );
        UI.SetValue(["Config", "AdvancedFeatures","AdvancedFeatures", misc_subtab2_name.sub5] , misc_subtab2_var.sub5[0] );
    }  
    ...
    switch( misc_subtab2_var.sub1[0] )
    {
        ...
        case 2:
            ...
            Render.String(start_x+1, start_y+3, 0, text, [0, 0, 0, 146], Render.AddFont(style.title_font, 12, 400));
            Render.String(start_x, start_y+2, 0, text, [255, 255, 255, 255], Render.AddFont(style.title_font, 12, 600));
...
UI.AddDropdown (["Config", "AdvancedFeatures","AdvancedFeatures"], misc_subtab3_name.sub1, ["1", "2","3"], 0);
UI.AddCheckbox (["Config", "AdvancedFeatures","AdvancedFeatures"], misc_subtab3_name.sub2);
UI.AddCheckbox (["Config", "AdvancedFeatures","AdvancedFeatures"], misc_subtab3_name.sub3);
...
function misc_subtab3()
{
    ...
    if( !misc_subtab3_last_state )
    {
        ...
        misc_subtab3_var.sub1[0] = UI.GetValue(["Config", "AdvancedFeatures","AdvancedFeatures", misc_subtab3_name.sub1]);        
        misc_subtab3_var.sub2 = UI.GetValue(["Config", "AdvancedFeatures","AdvancedFeatures", misc_subtab3_name.sub2]);
        misc_subtab3_var.sub3 = UI.GetValue(["Config", "AdvancedFeatures","AdvancedFeatures", misc_subtab3_name.sub3]);
        misc_subtab3_var.sub3_1 = get_key(misc_subtab3_name.sub3_1,misc_subtab3_var.sub3_1)
    }
    else
    {
        UI.SetValue(["Config", "AdvancedFeatures","AdvancedFeatures", misc_subtab3_name.sub1] , misc_subtab3_var.sub1[0] );
        UI.SetValue(["Config", "AdvancedFeatures","AdvancedFeatures", misc_subtab3_name.sub2] , bool_to_int(misc_subtab3_var.sub2));
        UI.SetValue(["Config", "AdvancedFeatures","AdvancedFeatures", misc_subtab3_name.sub3] , bool_to_int(misc_subtab3_var.sub3));
        UI.SetEnabled( ["Config", "AdvancedFeatures","AdvancedFeatures", misc_subtab3_name.sub1], 0 );
        UI.SetEnabled( ["Config", "AdvancedFeatures","AdvancedFeatures", misc_subtab3_name.sub2], 0 );
        UI.SetEnabled( ["Config", "AdvancedFeatures","AdvancedFeatures", misc_subtab3_name.sub3], 0 );
